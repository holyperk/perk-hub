--[[
    ...
    Default GUI key: RightAlt
    V14 transport fix using game:HttpGet
]]
...
local VERSION = "1.0"
local SCRIPT_URL = "https://perk-hub-api.mefistovmisha.workers.dev/release-v13"
local LOADER_TOKEN = 'V13_b792af072a1b466a9ae8de8bc04a2dd46a6a160b7f15d1db'
...
local function RequestAuth(key)
    ...
    -- Use Roblox's HttpGet for the Worker download instead of an executor
    -- request wrapper. Some executors return a blank Body or a misleading
    -- HTTP status for Worker responses even when the response is valid.
    local scriptUrl = SCRIPT_URL .. "?loader=" .. HttpService:UrlEncode(LOADER_TOKEN)

    local okScript, body = pcall(function()
        return game:HttpGet(scriptUrl)
    end)

    if not okScript or type(body) ~= "string" then
        return false, "failed to reach PERK HUB server"
    end
    ...
    if body == "Unauthorized" or body == "Not Found" then
        return false, "hub server rejected the loader"
    end

    if body:find("PERK HUB", 1, true) == nil then
        return false, "unexpected response from hub server"
    end
